...
# VP: Path to Excel file
EXCEL_PATH  <- "/Users/vpills/Documents/Invitae_Personal.xlsx"
SHEET_NAMES <- c(
...
# VP: Function to bucket specimen types into standard groups
bucket_specimen <- function(x) {
  x <- trimws(as.character(x))
  case_when(
    is.na(x) | x==""                  ~ "Unknown",
    x %in% c("Blood","Cord Blood")    ~ "Blood",
    x %in% c("Saliva","Assisted Saliva") ~ "Saliva",
    x %in% c("Buccal Swab")           ~ "Buccal Swab",
    x %in% c("gDNA - cultured fibroblasts") ~ "Fibroblast DNA",
    x %in% c("gDNA")                  ~ "Other gDNA",
    TRUE                              ~ "Other"
  )
}
...
read_and_harmonise <- function(path, sheet) {
  ...
  df <- df |>
    mutate(
      ...
      specimen_type_bucketed  = bucket_specimen(specimen_type),
...
apply_filters <- function(df, group_name, is_vus_group = FALSE, is_possibly_mosaic = FALSE) {
  ...
  # VP: Deduplicate per participant×variant
  df <- df |>
    mutate(
...
clean_poss <- apply_filters(df_poss, "Possibly Mosaic", is_vus_group = FALSE, is_possibly_mosaic = TRUE)
clean_plp  <- apply_filters(df_plp, "P/LP",            is_vus_group = FALSE, is_possibly_mosaic = FALSE)
clean_vus  <- apply_filters(df_vus, "Non-P/LP (VUS)",  is_vus_group = TRUE,  is_possibly_mosaic = FALSE)

out_dir <- "/Users/vpills/Documents/ukb_outputs"
dir.create(out_dir, showWarnings = FALSE)

# ---- Demographics + Specimen Combined Summary ----
...
mk_specimen_tbl <- function(df, label) {
  id_col <- if ("dedup_id" %in% names(df) && COUNT_UNIQUE_PARTICIPANTS) "dedup_id" else NULL
  demog <- if (!is.null(id_col)) df |> arrange(.data[[id_col]]) |> distinct(.data[[id_col]], .keep_all = TRUE) else df

  demog |>
    mutate(specimen_type_bucketed = ifelse(is.na(specimen_type_bucketed) | specimen_type_bucketed=="", "Unknown", specimen_type_bucketed)) |>
    count(specimen_type_bucketed, name = "n") |>
    mutate(
      pct = 100 * n / sum(n),
      fmt = sprintf("%d (%.1f%%)", n, pct),
      Characteristic = paste0("  ", specimen_type_bucketed)
    ) |>
    select(Characteristic, !!label := fmt)
}

# Creating a put-together table
summary_tbl <- mk_summary(clean_plp, "P/LP") |>
  ...
  bind_rows(
    tibble::tibble(
      Characteristic = "Specimen Type",
      `P/LP` = "",
      `Non-P/LP (VUS)` = "",
      `Possibly Mosaic` = ""
    ),
    mk_specimen_tbl(clean_plp, "P/LP") |>
      full_join(mk_specimen_tbl(clean_vus, "Non-P/LP (VUS)"),  by="Characteristic") |>
      full_join(mk_specimen_tbl(clean_poss, "Possibly Mosaic"), by="Characteristic")
  )
write_csv(summary_tbl, file.path(out_dir, "Summary_Table.csv"))
tg <- gridExtra::tableGrob(summary_tbl, rows = NULL)
ggsave(file.path(out_dir, "Summary_Table.png"), tg, width = 10, height = 7, dpi = 300, units = "in")
